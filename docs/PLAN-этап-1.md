...
- [x] **Шаг 1.** Тест: `REGISTRY` покрывает все нетерминальные состояния и ни одного терминального
- [x] **Шаг 2.** Тест на каждый переход отдельно (8 тестов): пост в состоянии X, вызвать обработчик, проверить `outcome == ADVANCED`, `next_state` правильный, и побочный эффект на месте:
...
- [x] **Шаг 3.** Тест идемпотентности `images`: если у половины `assets` уже есть `local_path`, повторный вызов не трогает их (проверить по `mtime` файла и по числу вызовов провайдера)
- [x] **Шаг 4.** Тест идемпотентности `publish`: пост с непустым `external_id` не вызывает `publisher.publish` повторно
- [x] **Шаг 5.** Запустить, убедиться что падает
- [x] **Шаг 6.** Реализовать `steps/__init__.py` (`Outcome`, `StepResult`, `StepContext`, `REGISTRY`) и семь шагов. Шаги вызывают провайдеров через `ctx.providers`, никакой сетевой специфики внутри
- [x] **Шаг 7.** Тесты зелёные, коммит: `feat: шаги пайплайна на stub-провайдерах`
...
- [x] **Шаг 1.** Тест: 10 свободных тем, `queue_buffer=3` (задаётся в тесте, чтобы проверить, что значение читается из конфига, а не зашито) → после `replenish_queue` ровно 3 поста, 3 темы в статусе `taken`
- [x] **Шаг 2.** Тест: повторный вызов `replenish_queue` не создаёт постов сверх буфера
- [x] **Шаг 3.** Тест: посты в `published`, `failed` и `rejected` в буфере не считаются и освобождают место под новые
- [x] **Шаг 4.** Тест: свободных тем нет → предупреждение в логе, исключения нет
- [x] **Шаг 5.** Тест: `idem_key` создаваемого поста равен `demo:<topic_id>:0`
- [x] **Шаг 6.** Запустить, убедиться что падает
- [x] **Шаг 7.** Реализовать `replenish_queue(conn, project)` и `claim_free_topic(conn, project_id)` (атомарный `UPDATE topics SET status='taken' WHERE id = (SELECT id FROM topics WHERE project_id=? AND status='free' ORDER BY id LIMIT 1) RETURNING id`)
- [x] **Шаг 8.** Тесты зелёные, коммит: `feat: пополнение очереди постов до буфера`
...
- [x] **Шаг 1.** Тест: `FACTORY_MAX_STEPS_PER_TICK=1` → один тик двигает пост ровно на одно состояние
- [x] **Шаг 2.** Тест: `FACTORY_MAX_STEPS_PER_TICK=3` → один тик двигает пост из `queued` в `prompts_ready`
- [x] **Шаг 3.** Тест: шаг вернул `WAITING` → цепочка обрывается, `retry_count` не вырос, состояние не изменилось
- [x] **Шаг 4.** Тест: второй шаг цепочки бросил исключение → первый переход сохранён в базе, `retry_count == 1`, `next_attempt_at` в будущем
- [x] **Шаг 5.** Тест backoff: паузы после отказов идут 10, 20, 40, 80 минут — ровно четыре значения. Пятый отказ переводит пост в `failed`, и пауза 160 минут не применяется никогда: `next_attempt_at` у поста в `failed` смысла не имеет. Проверить это явно, иначе формула и порог живут своей жизнью
- [x] **Шаг 6.** Тест: посты с `next_attempt_at` в будущем тиком не берутся
- [x] **Шаг 7.** Запустить, убедиться что падает
- [x] **Шаг 8.** Реализовать `advance_post()`, `record_failure()`, `record_wait()`, `commit_transition()`, `due_posts()`, `tick()` (полный цикл: блокировка → по всем активным проектам пополнить очередь → продвинуть посты → хартбит)
- [x] **Шаг 9.** Тесты зелёные, коммит: `feat: многошаговый тик стейт-машины с backoff`
...
- [x] **Шаг 1.** Тест: `posts_per_day=2`, два поста уже `published` сегодня → третий получает `WAITING`, а не ошибку, `retry_count` не растёт
- [x] **Шаг 2.** Тест: счётчик считает по `published_at`, а не по `updated_at`. Пост опубликован вчера в 23:50 по Москве, сегодня его строку тронули (например, `post retry` обновил `updated_at`) — сегодняшний лимит он НЕ занимает
- [x] **Шаг 2а.** Тест: счётчик считает в часовом поясе проекта, а не в UTC. Пост, опубликованный в 23:30 по Москве, относится к московским суткам, хотя в UTC это уже 20:30 тех же суток — а для `Asia/Vladivostok` тот же момент попадёт в следующие сутки
- [x] **Шаг 3.** Тест: время вне слотов расписания → `WAITING("вне расписания публикаций")`
- [x] **Шаг 4.** Тест: `FACTORY_IGNORE_SCHEDULE=1` → проверка расписания пропускается
- [x] **Шаг 5.** Тест: при `FACTORY_IGNORE_SCHEDULE=1` каждый тик пишет в лог запись уровня WARNING со словами «расписание отключено»
- [x] **Шаг 6.** Запустить, убедиться что падает
- [x] **Шаг 7.** Реализовать `published_today(conn, project)` (по `published_at`) и `schedule_slot_open(project, now)` (окно: от времени слота до слота + 60 минут, слот считается использованным, если в нём уже была публикация); WARNING про отключённое расписание пишется в `machine.tick()`, а не в шаге, — иначе он не появится, когда публиковать нечего
- [x] **Шаг 8.** Тесты зелёные, коммит: `feat: расписание публикаций и дневной лимит`
...
- [x] **Шаг 1.** Тест: `reject_post(id, reason="trash")` → пост в `rejected`, тема снова `free`, `used_at` очищен, в `rejections` появилась строка со снапшотом (`title`, `body`, промпты)
- [x] **Шаг 2.** Тест переиспользования: после отклонения `replenish_queue` берёт ту же тему и создаёт по ней новый пост
- [x] **Шаг 3.** Тест: `idem_key` нового поста по той же теме равен `demo:<topic_id>:1` — уникальный индекс не срабатывает, `IntegrityError` не возникает
- [x] **Шаг 4.** Тест: тему отклонили трижды → четвёртый пост получает `attempt = 3`, все четыре строки в `posts` живы
- [x] **Шаг 5.** Запустить, убедиться что падает
- [x] **Шаг 6.** Реализовать `reject.py`. Формат `idem_key` — `f"{slug}:{topic_id}:{attempt}"`, где `attempt` = число прошлых отклонений темы (`SPEC.md` уже обновлён). Защита от дубля публикации держится не на этом ключе, а на проверке `external_id IS NULL` перед вызовом публикации
- [x] **Шаг 7.** Тесты зелёные, коммит: `feat: отклонение поста с возвратом темы в очередь`
...
- [x] **Шаг 1.** Тест через `typer.testing.CliRunner`: `factory init` создаёт файл базы и применяет миграции; повторный вызов не ломается
- [x] **Шаг 2.** Тест: `factory project add demo` регистрирует проект; повторный вызов даёт понятное сообщение, а не трейсбек
- [x] **Шаг 3.** Тест: `factory topics import demo <файл>` загружает темы построчно, пустые строки и дубли пропускает, печатает сколько загружено
- [x] **Шаг 4.** Тест: `factory run --once` отрабатывает один тик и завершается с кодом 0
- [x] **Шаг 5.** Тест: `factory post show <id>` печатает состояние, заголовок, число картинок, последнюю ошибку
- [x] **Шаг 6.** Тест: `factory doctor` на здоровой системе даёт код 0; при отсутствии базы — код 1 и текст «что делать»
- [x] **Шаг 6а.** Тест: `factory doctor` при недоступном на запись каталоге данных печатает подсказку про `FACTORY_DATA_DIR` и завершается с кодом 1 — без `PermissionError` и трейсбека. `doctor` вызывает `ensure_data_dir()` первым, до попытки открыть базу
- [x] **Шаг 7.** Запустить, убедиться что падает
- [x] **Шаг 8.** Реализовать `workers/tick.py` (`run_once()`, `run_loop()` со `sleep(tick_interval_sec())` и корректной обработкой `SIGTERM`) и `cli.py` на typer: `init`, `doctor`, `unlock`, `run`, группы `project` (`add`, `list`), `topics` (`import`, `list`), `post` (`create`, `show`, `list`, `retry`, `reject`). У каждой команды — русская строка справки; все ошибки ловятся и печатаются как `FactoryError`, без трейсбеков
- [x] **Шаг 9.** Тесты зелёные, коммит: `feat: воркер тика и CLI на typer`
...
- [x] **Шаг 1.** Тест возобновляемости: запустить `factory run --loop` подпроцессом при `FACTORY_MAX_STEPS_PER_TICK=3` и заглушке, которая на третьем шаге зависает; убить `SIGKILL`; проверить, что первые два перехода в базе сохранены; запустить `factory run --once` и убедиться, что пост поехал дальше с третьего шага, а не с начала
- [x] **Шаг 2.** Тест «оплаченное не переделывается»: после краша на шаге `images` уже сгенерированные файлы не перегенерируются — счётчик вызовов stub-провайдера это подтверждает
- [x] **Шаг 3.** Тест блокировки тика: запустить два `factory run --once` одновременно (`multiprocessing`); проверить, что постов создано ровно `queue_buffer`, каждая тема занята не более одного раза, и ни один пост не опубликован дважды
...
- [x] **Шаг 5.** Тест перехвата протухшей блокировки: убитый процесс оставил `tick_lock`; при `FACTORY_LOCK_TTL_SEC=1` следующий тик её забирает
- [x] **Шаг 6.** Все тесты зелёные, коммит: `test: возобновляемость после краша и защита от параллельных тиков`
...
- [x] **Шаг 1.** `README.md`: что это, установка с нуля на macOS (реальные команды, без плейсхолдеров), первый запуск до первого «опубликованного» поста-заглушки, что появится на следующих этапах. Для локальной разработки указать конкретные значения, готовые к копипасту: `export FACTORY_DATA_DIR=~/factory-data`, `export FACTORY_PROJECTS_DIR=$(pwd)/projects` — умолчания `/data` и `/app/projects` рассчитаны на контейнер и на macOS без `sudo` не работают
- [x] **Шаг 2.** `RUNBOOK.md` — рецепты, доступные на этом этапе: запустить/остановить, посмотреть что происходит сейчас, посмотреть логи, поставить на паузу, добавить темы, посмотреть сколько тем осталось, отклонить пост, перезапустить застрявший пост, снять зависшую блокировку, сделать бэкап базы
- [x] **Шаг 3.** `TROUBLESHOOTING.md`: таблица симптом → причина → команда проверки → лечение. Минимум пять строк из реальных ситуаций этапа (посты не двигаются; «темы закончились»; блокировка висит; база заблокирована; конфиг не проходит валидацию)
- [x] **Шаг 4.** `CLAUDE.md`: архитектура в двух абзацах, карта каталогов, конвенции, как добавить шаг пайплайна, как добавить провайдера, как гонять тесты
- [x] **Шаг 5.** `CLAUDE.md`, раздел «Задолженность Этапа 5» — записать обе задачи дословно:
...
- [x] **Шаг 6.** Коммит: `docs: README, RUNBOOK, TROUBLESHOOTING, CLAUDE.md для Этапа 1`

---

### Задача 17: приёмка этапа

- [x] **Шаг 1.** `uv run pytest -v` — все тесты зелёные, ни одного пропущенного
- [x] **Шаг 2.** Проверить изоляцию от сети: `uv run pytest tests/test_no_network.py -v` — предохранитель из `conftest.py` жив и роняет любой реальный запрос с понятным сообщением
- [x] **Шаг 3.** Сквозной прогон по рецепту из `README.md` при `FACTORY_TICK_INTERVAL_SEC=5`, `FACTORY_IGNORE_SCHEDULE=1`: база → проект → 10 тем → `run --loop` → через минуту два поста в `published`
- [x] **Шаг 4.** Проверка глазами: `factory post show 1` показывает заголовок, текст, 4 файла картинок на диске, фейковый `external_id`
- [x] **Шаг 5.** Проверка возобновляемости руками: `run --loop`, `kill -9` на середине, повторный запуск — пост доезжает
- [x] **Шаг 6.** Проверка приёмки по спеке: человек, не открывая исходники, выполняет рецепты из `RUNBOOK.md` и получает ожидаемый результат
- [x] **Шаг 7.** Финальный коммит и тег: `git tag stage-1`

---

## Что изменилось по итогам финального ревью

Ревьюер с чистым контекстом прошёл диапазон коммитов целиком. Пять серьёзных
находок и десяток мелких; все приняты и закрыты.

| Что было | Чем это грозило |
|----------|-----------------|
| `kill -9` в тестах бил по `uv`, а не по воркеру | три приёмочных теста этапа не проверяли ничего и оставляли бессмертные процессы |
| `kill -9` в RUNBOOK — тот же дефект | владелец снял бы блокировку при живом воркере и получил два процесса сразу |
| `build_providers` вне общего `try` | один битый провайдер останавливал все проекты и хартбит |
| точка входа вела на `app`, а не на `cli` | `factory run --loop` печатал трейсбек вместо инструкции |
| тест «пост не публикуется дважды» был тавтологией | `external_id` заглушки уникален по построению |
| окно слота обрывалось о полночь | слот 23:55 имел реальное окно в пять минут |
| `factory stats --days` не работал | период игнорировался, деньги считались по всем проектам |
| WARNING «темы закончились» на каждом тике | 144 строки в сутки топили совет «смотри WARNING в логах» |
| `assert` для сверки реестра и карты переходов | исчезал под `python -O` |

Плюс восемь дыр в тестах, найденных мутациями: правило фактчека `fixed`,
`factcheck_notes`, обрезка по `inline_count`, разные `seed`, снятие паузы в
`post retry`, предохранитель воркера, `PRAGMA foreign_keys` и `busy_timeout`,
ветка «нет записи обложки».
