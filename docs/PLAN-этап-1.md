...
| 2 | `queue_buffer` (по умолчанию 3) — сколько постов держать в работе одновременно | развязывает наполнение очереди и публикацию |
...
| 5 | Тема занимается в момент **создания** поста, а не в шаге `text` | `posts.topic_id` обязателен, `idem_key = slug:topic_id` |
...
| Env `FACTORY_IGNORE_SCHEDULE` (`0`) | только для разработки: пропустить проверку расписания, иначе сквозной прогон невозможен |
...
| `factory/core/paths.py` | все пути и числовые лимиты из env со значениями по умолчанию |
...
```python
delay_sec = min(600 * 2 ** (retry_count - 1), 21600)   # 10, 20, 40, 80, 160 мин; потолок 6 ч
...
replenish_queue(project):
    ...
    while in_flight < project.limits.queue_buffer:
        ...
        create_post(project, topic)            # idem_key = f"{slug}:{topic.id}"
        in_flight += 1
```
...
publish step:
    ...
    published_today = COUNT posts WHERE project_id = ? AND state = 'published'
                                    AND date(updated_at в tz проекта) = сегодня
...
- [ ] **Шаг 1.** `tests/conftest.py`: фикстура `tmp_env` — подменяет `FACTORY_DATA_DIR`/`FACTORY_TMP_DIR`/`FACTORY_PROJECTS_DIR` на временные каталоги; фикстура `conn` — открытая база с применёнными миграциями
...
- [ ] **Шаг 4.** Написать `migrations/001_init.sql`: вся схема из спеки, плюс `meta(key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TEXT NOT NULL)`, плюс колонки `posts.factcheck_verdict TEXT`, `posts.factcheck_notes TEXT`, плюс индексы из спеки
...
- [ ] **Шаг 1.** Тест: `projects/demo/config.yaml` загружается, `cfg.limits.queue_buffer == 3`, `cfg.limits.posts_per_day == 2`, `cfg.review.mode == "auto"`
...
- [ ] **Шаг 1.** Тест: первый `acquire_tick_lock()` успешен, второй при живой блокировке — возвращает `None`
- [ ] **Шаг 2.** Тест: блокировка с истёкшим `expires_at` перехватывается новым тиком
- [ ] **Шаг 3.** Тест: выход из контекст-менеджера снимает блокировку даже при исключении внутри
- [ ] **Шаг 4.** Тест: `write_heartbeat()` и `heartbeat_age_sec()` работают
- [ ] **Шаг 5.** Запустить, убедиться что падает
- [ ] **Шаг 6.** Реализовать `lock.py`: `tick_lock()` — контекст-менеджер поверх строки `meta['tick_lock']` (JSON `{host, pid, expires_at}`), захват и перехват протухшей в одной транзакции; `refresh()` — продлить; `force_unlock()` — для CLI; `write_heartbeat()` / `heartbeat_age_sec()`
- [ ] **Шаг 7.** Тесты зелёные, коммит: `feat: блокировка тика с TTL и хартбит`
...
- [ ] **Шаг 1.** Тест: успешный вызов пишет строку в `runs` с `ok=1` и ненулевым `duration_ms`
- [ ] **Шаг 2.** Тест: функция, падающая с сетевой ошибкой дважды и успешная на третий раз, — отрабатывает успешно, в `runs` одна строка `ok=1`
- [ ] **Шаг 3.** Тест: `httpx.HTTPStatusError` со статусом 429 и заголовком `Retry-After: 1` приводит к паузе перед повтором (пауза мокается, проверяется вызов)
- [ ] **Шаг 4.** Тест: исчерпание попыток пишет `runs.ok=0` с текстом ошибки и пробрасывает исключение наверх
- [ ] **Шаг 5.** Запустить, убедиться что падает
- [ ] **Шаг 6.** Реализовать `retry.py`: декоратор `tracked_call(step_name)` — 3 попытки на сетевые ошибки и 429 (с уважением `Retry-After`), экспоненциальная пауза, замер времени, запись в `runs` (включая `cost_usd`, если функция вернула его в атрибуте результата)
- [ ] **Шаг 7.** Тесты зелёные, коммит: `feat: единый декоратор ретраев и учёта вызовов`
...
- [ ] **Шаг 2.** Тест на каждый переход отдельно (8 тестов): пост в состоянии X, вызвать обработчик, проверить `outcome == ADVANCED`, `next_state` правильный, и побочный эффект на месте:
  ...
  - `publish` → заполнен `external_id`, тема переведена в `used`
...
- [ ] **Шаг 1.** Тест: 10 свободных тем, `queue_buffer=3` → после `replenish_queue` ровно 3 поста, 3 темы в статусе `taken`
- [ ] **Шаг 2.** Тест: повторный вызов `replenish_queue` не создаёт постов сверх буфера
- [ ] **Шаг 3.** Тест: опубликованный пост освобождает место в буфере, и создаётся новый
- [ ] **Шаг 4.** Тест: свободных тем нет → предупреждение в логе, исключения нет
- [ ] **Шаг 5.** Тест: `idem_key` создаваемого поста равен `demo:<topic_id>`
- [ ] **Шаг 6.** Запустить, убедиться что падает
...
- [ ] **Шаг 5.** Тест backoff: паузы после отказов идут 10, 20, 40, 80, 160 минут; на пятом отказе состояние становится `failed`
...
- [ ] **Шаг 2.** Тест: счётчик считает по часовому поясу проекта, а не по UTC (пост, опубликованный в 23:30 по Москве, относится к московским суткам)
...
- [ ] **Шаг 5.** Запустить, убедиться что падает
- [ ] **Шаг 6.** Реализовать `published_today(conn, project)` и `schedule_slot_open(project, now)` (окно: от времени слота до слота + 60 минут, слот считается использованным, если в нём уже была публикация)
- [ ] **Шаг 7.** Тесты зелёные, коммит: `feat: расписание публикаций и дневной лимит`
...
- [ ] **Шаг 3.** Тест: `idem_key` нового поста конфликтует со старым (тот же `slug:topic_id`) — проверить, что это обработано осмысленно, а не падением с `IntegrityError`
- [ ] **Шаг 4.** Запустить, убедиться что падает
- [ ] **Шаг 5.** Реализовать `reject.py`. Решение по конфликту `idem_key`: к ключу добавляется порядковый номер попытки — `f"{slug}:{topic_id}:{attempt}"`, где `attempt` = число прошлых отклонений темы. Защита от дубля публикации при этом сохраняется, потому что она держится на проверке `external_id IS NULL`
- [ ] **Шаг 6.** Тесты зелёные, коммит: `feat: отклонение поста с возвратом темы в очередь`

> Замечание: строку про формат `idem_key` нужно поправить и в `SPEC.md` — иначе через месяц
> никто не вспомнит, откуда взялся третий сегмент.
...
- [ ] **Шаг 3.** Тест параллельности: запустить два `factory run --once` одновременно (`multiprocessing`); проверить, что постов создано ровно `queue_buffer`, каждая тема занята не более одного раза, и ни один пост не опубликован дважды
- [ ] **Шаг 4.** Тест перехвата протухшей блокировки: убитый процесс оставил `tick_lock`; при `FACTORY_LOCK_TTL_SEC=1` следующий тик её забирает
- [ ] **Шаг 5.** Все тесты зелёные, коммит: `test: возобновляемость после краша и защита от параллельных тиков`
...
- [ ] **Шаг 1.** `README.md`: что это, установка с нуля на macOS (реальные команды, без плейсхолдеров), первый запуск до первого «опубликованного» поста-заглушки, что появится на следующих этапах
...
- [ ] **Шаг 5.** Коммит: `docs: README, RUNBOOK, TROUBLESHOOTING, CLAUDE.md для Этапа 1`
...
- [ ] **Шаг 2.** Проверить, что реальная сеть в тестах не дёргается: `uv run pytest -p no:randomly` при отключённом интернете проходит
