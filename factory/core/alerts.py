...
def provider_blocked_text(project: str, error: str) -> str:
    """Провайдер отказал так, что снять отказ может только человек.

    Кнопки «попробовать снова» здесь нет намеренно: пока лимит не поднят, повтор
    вернёт ровно этот же отказ. Обещать действие, которое не сработает, — то же
    самое, что соврать.
    """
    return (
        f"⛔️ [{project}] Работа встала: провайдер отказывает.\n\n"
        f"{error.strip()[:900]}\n\n"
        "Посты не потеряны: они подождут и пойдут дальше сами, как только "
        "причина исчезнет."
    )


def budget_exceeded_text(
    project: str, post_id: int, title: str | None, spent: float, limit: float
) -> str:
    """Пост стоил дороже потолка. Здесь важно назвать обе цифры.

    Владелец не читает код, и «превышен лимит» без чисел не даёт ему решить,
    поднимать потолок или выбрасывать пост.
    """
    return (
        f"💸 [{project}] Пост {post_id} остановлен: он вышел дороже потолка.\n\n"
        f"«{title or 'без заголовка'}»\n\n"
        f"Потрачено {spent:.2f} при потолке {limit:.2f} за пост.\n\n"
        "Что делать: либо поднять limits.max_cost_per_post в конфиге проекта, "
        "либо выбросить пост. Повтор без этого упрётся в тот же потолок: "
