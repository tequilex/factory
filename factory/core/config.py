...
class VkCfg(_Section):
    ...
    # Идентификатор приложения ВК, через которое выдаётся ключ загрузки. Нужен
    # только для ссылки, которую бот присылает при истечении ключа. В коде его
    # быть не может: приложение заводит владелец, и у каждой группы оно своё.
    app_id: int | None = None
    # Защищённый ключ приложения. Нужен, чтобы менять одноразовый код на ключ
    # загрузки — тогда ключ выписывается на адрес системы, и обновлять его
    # можно откуда угодно. Без него остаётся старая схема: готовый ключ из
    # браузера, работающая только с того же адреса.
