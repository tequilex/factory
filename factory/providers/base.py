...
#: Ответ провайдера про исчерпанный лимит ключа или квоту.
#:
#: Совпадение по подстроке, потому что код у такого отказа обычный 429 — тот же,
#: что у «слишком часто стучитесь». Разница принципиальная: частоту переживёт
#: повтор через минуту, а лимит не снимется сам никогда, и попытки поста уйдут
#: на ожидание события, которое без человека не наступит.
_SPENDING_LIMIT_MARKS = ("spending limit", "лимит", "quota")


def is_spending_limit(body: str) -> bool:
    """Похож ли ответ на отказ по исчерпанному лимиту ключа.

    Живёт здесь, а не в каком-то одном провайдере, потому что ловушка общая:
    текст и картинки берутся у одного реселлера, по одному ключу, и лимит у них
    тоже один. Проверено живьём — сначала на картинках, потом на текстах.
    """
    lowered = body.lower()
